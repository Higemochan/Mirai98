...
// and the Towns gamepad on a TOWNS console.  The PC-98 side is untouched:
// the console hook only engages for machine === "towns".
...
// ---- which console belongs to a TOWNS machine -----------------------------
// Only the gamepad watcher is Towns-specific, and it needs the machine kind
// before the console hook runs.  The relative-pointer capture that used to
// live here moved to the core: every machine the manager runs has a relative
// mouse, so the core captures the pointer on every console.
...
window.registerMachinePlugin({
  ...
  // the machine kind is settled here, before the console hook runs, so the
  // pad watcher knows synchronously whether this console is a Towns one
