...
TREE_DIR=$(CDPATH= cd -- "$BASE_DIR/.." && pwd)
DOWNLOAD_DIR="$BASE_DIR/downloads"
SOURCE_DIR="$BASE_DIR/deps-src"
DEPS_BUILD_DIR="$BASE_DIR/deps-build"
ROOT_DIR="$BASE_DIR/root"
# The qemu-pc98 submodule, so this and the Live USB are always the same
# commit.  The only thing written into it is subprojects/keycodemapdb,
# which QEMU's own .gitignore covers, so the submodule stays clean.
QEMU_SOURCE_DIR=${QEMU_SOURCE_DIR:-"$TREE_DIR/qemu-pc98"}
QEMU_BUILD_DIR="$BASE_DIR/build"
STAGE_DIR="$BASE_DIR/stage"
# What the distribution directory, and so the folder inside the zip, is
# called.  "release" sets it to the published name; on its own the dist
# stage uses a neutral one.
DIST_NAME=${DIST_NAME:-qemu-pc98-bin}
...
CROSS_FILE="$BASE_DIR/cross-mingw.ini"
CMAKE_TOOLCHAIN="$BASE_DIR/toolchain-mingw.cmake"
...
{
    local name
    # virtpc98.py comes from src/, the one copy in the tree and the same
    # file the appliance ships, rather than a second one that can fall
    # behind it without anyone noticing
    [[ -f "$TREE_DIR/src/virtpc98.py" ]] ||
        die "virtpc98.py not found at $TREE_DIR/src/virtpc98.py"
    cp -p "$TREE_DIR/src/virtpc98.py" "$DIST_DIR/virtpc98.py"
    # virtpc98.exe is built by PyInstaller on Windows, so it cannot be
    # produced here and has to be dropped into package-assets/ by hand
    for name in README.txt virtpc98.exe; do
...
make_dist()
{
    ...
    local suffix
    local qemu_commit
    local archive="$BASE_DIR/$DIST_NAME.zip"
    local -a exes=()
    local -a qemu_exes=()
...
make_release()
{
    ...
    [[ -n "$rev" ]] ||
        die "usage: $0 release REV   (for example: $0 release rev11)"
    version=$(cut -d. -f1,2 "$QEMU_SOURCE_DIR/VERSION")
    name="qemu-$version-pc98-$rev"

    # re-enter with the published name, so the directory inside the zip
    # is the one people unpack rather than a build-internal one
    DIST_NAME="$name" "$0" all

    cat <<EOF

==> $name.zip is ready in win64/

Nothing has been uploaded.  To publish it as a qemu-pc98 release:

  gh release create $rev --repo awemorris/qemu-pc98 \\
      --title "QEMU-$version with PC-9821 patch $rev" \\
      --notes-file NOTES.md \\
      win64/$name.zip

or, to attach it to a release that already exists:

  gh release upload $rev --repo awemorris/qemu-pc98 win64/$name.zip
EOF
}
...
Commands:
  ...
  dist         Assemble \$DIST_NAME/ and \$DIST_NAME.zip
  verify       Inspect PE architecture, checksums and DLL closure
  all          Run deps, qemu, dist and verify
  release REV  Run all under the published name, then print how to
               upload it (for example: $0 release rev11)

The QEMU source is the qemu-pc98 submodule; \$QEMU_SOURCE_DIR overrides it.
EOF
}
